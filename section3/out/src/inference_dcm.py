...
def get_predicted_volumes(pred):
    ...
    # <YOUR CODE HERE>
    return {"anterior": np.sum(pred == 1), 
            "posterior": np.sum(pred == 2), 
            "total": np.sum(pred > 0)}
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    selected_slice = orig_vol[0, :, :]
    nd_img = np.flip((selected_slice/np.max(selected_slice))*0xff).T.astype(np.uint8)
...
if __name__ == "__main__":
    ...
    study_dir = [d for d in sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True) if 'HCropVolume' in d][0]
